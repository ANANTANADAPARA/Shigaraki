import subprocess                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     marcos.py                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 import subprocess
import re
import pyfiglet
...
def scan_subdomains_with_httpx(subdomains, speed_factor=10):
    try:
        # Prepare subdomains file for httpx input
        with open('subdomains.txt', 'w') as f:
            for subdomain in subdomains:
                if subdomain:
                    f.write(f"https://{subdomain}\n")
        
        # Run httpx-toolkit command to scan subdomains
        result = subprocess.run(['httpx-toolkit', '-l', 'subdomains.txt', '-threads', str(speed_factor), '-timeout', '5'], capture_output=True, text=True)
        urls = result.stdout.splitlines()
        
        return urls
    except Exception as e:
        print(f"Error scanning subdomains with httpx: {e}")
        return []

def add_admin_login_register_paths(urls):
   
    paths = ['login','register','admin','admin/','administrator/','login.php','administration/','admin1/','admin2/','admin3/','admin4/','admin5/','moderator/','webadmin/','adminarea/','bb-admin/','adminLogin/','admin_area/','panel-administracion/','instadmin/',
...
def check_panels_with_halive(urls):
    try:
        # Prepare URLs file for halive  input
        with open('urls_with_paths.txt', 'w') as f:
            for url in urls:
                f.write(f"{url}\n")
        
        # Run halive command to check if panels exist
        result = subprocess.run(['halive','urls_with_paths.txt','-t', '300' ,'--only-success' ,'--only-urls'], capture_output=True, text=True)
        live_urls = result.stdout.splitlines()
        
        return live_urls
    except Exception as e:
        print(f"Error checking panels with halive: {e}")
        return []

def main():
    
    ascii_banner = pyfiglet.figlet_format("SHIGARAKI")
    print(ascii_banner)
    print("\033[1m                           MADE BY ANANTA<33                \033[0m")
    domain = input("Enter the domain to scan: ")
    speed_factor = input("Enter speed factor (default 10): ")
    try:
        speed_factor = int(speed_factor) if speed_factor else 10
    except ValueError:
        print("Invalid speed factor. Defaulting to 10.")
        speed_factor = 10
    
    subdomains = find_subdomains(domain)
    if subdomains:
        print(f"Found {len(subdomains)} subdomains.")
        print("Scanning subdomains with httpx-toolkit...")
        urls = scan_subdomains_with_httpx(subdomains, speed_factor)
        
       
        urls_with_paths = add_admin_login_register_paths(urls)
        
        print("Checking panels with halive...")
        live_urls = check_panels_with_halive(urls_with_paths)
        
        if live_urls:
            print("Found live panels:")
            for url in live_urls:
                print(url)
        else:
            print("No live panels found.")
    else:
